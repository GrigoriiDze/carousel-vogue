...
# 1. Загрузка фона (ТЕПЕРЬ МОЖНО МНОГО ФАЙЛОВ)
uploaded_bgs = st.file_uploader(
    "1. Загрузи фоны (можно выбрать сразу несколько)", 
    type=['png', 'jpg', 'jpeg'], 
    ...
    if not uploaded_bgs:
        ...
        with st.spinner("Рендерим кадры..."):
            ...
            for i, slide in enumerate(slides_data):
                # --- ВЫБОР ФОНА ДЛЯ ТЕКУЩЕГО СЛАЙДА ---
                # Берем картинку по индексу. Если фоток меньше, чем текста — идем по кругу
                ...
                # Умный кроп (теперь применяется к каждой фотке индивидуально)
